...
- **Email — built-in, no SMTP required**: `insforge.emails.send()` and all auth verification/password-reset emails work out of the box on every paid project. **Do NOT install `nodemailer`, `resend`, `sendgrid`, `mailgun`, `postmark`, or any other third-party email package, and do NOT ask the user for SMTP credentials.** Only configure a custom sender domain when the user explicitly requests their own `from` address — and that config lives on the dashboard, not in `package.json`. See [email/sdk-integration.md](email/sdk-integration.md).
