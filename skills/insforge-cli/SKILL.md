...
> 💤 **Scale-to-zero is the default.** Every compute service deploys with Fly's autostop fully on: `autostop: "stop"` + `autostart: true` + `min_machines_running: 0`. (Those are the **Machines API** field names — fly.toml uses the longer `auto_stop_machines` / `auto_start_machines` for the same thing. We hit the Machines API directly, so the short names are what land in the request body.) When traffic is idle the machine stops; the next incoming request wakes it (~1s cold start on `shared-1x`). This is the most aggressive cost-saving setting Fly offers — the CLI and skill cannot dial it down further. The only direction to adjust is *up* (more warm capacity): always-on or `N` warm replicas. A flag to opt out is not yet exposed via the CLI; if you need always-on for a latency-critical service, contact support. Semantics mirror Fly.io exactly — `autostop` accepts `off | stop | suspend` and `min_machines_running` is an integer ≥ 0.
