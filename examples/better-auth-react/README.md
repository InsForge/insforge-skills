...
- **`lib/insforge.ts`** — the framework-agnostic `useInsforgeClient` hook. Uses a `setBridgeToken` helper that feature-detects the public `client.setAccessToken()` (lands in SDK 1.3.0) and falls back to the dual-update on the current 1.2.x — keeps HTTP and realtime in sync.
